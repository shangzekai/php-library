 ...
 * @package		Generate_PHPExcel_Chart_Renderer
...
require_once(Generate_PHPExcel_Settings::getChartRendererPath().'/jpgraph.php');


/**
 * Generate_PHPExcel_Chart_Renderer_jpgraph
 *
 * @category	Generate_PHPExcel
 * @package		Generate_PHPExcel_Chart_Renderer
 * @copyright	Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Chart_Renderer_jpgraph
{
	...
	private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '') {
		...
		foreach($datasetLabels as $i => $datasetLabel) {
			if (is_array($datasetLabel)) {
				...
				if (!is_null($datasetLabelFormatCode)) {
					$datasetLabels[$i] = Generate_PHPExcel_Style_NumberFormat::toFormattedString($datasetLabel,$datasetLabelFormatCode);
				}
			}
			++$testCurrentIndex;
		}
	...
	/**
	 * Create a new Generate_PHPExcel_Chart_Renderer_jpgraph
	 */
	public function __construct(Generate_PHPExcel_Chart $chart)
	{
		$this->_graph	= null;
		$this->_chart	= $chart;
