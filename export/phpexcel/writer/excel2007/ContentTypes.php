...
/**
 * Generate_PHPExcel_Writer_Excel2007_ContentTypes
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Writer_Excel2007_ContentTypes extends Generate_PHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeContentTypes(Generate_PHPExcel $pGenerate_PHPExcel = null, $includeCharts = FALSE)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			// Workbook
			if($pGenerate_PHPExcel->hasMacros()){ //Macros in workbook ?
				...
				if($pGenerate_PHPExcel->hasMacrosCertificate()){// signed macros ?
			...
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				$drawings = $pGenerate_PHPExcel->getSheet($i)->getDrawingCollection();
				$drawingCount = count($drawings);
				$chartCount = ($includeCharts) ? $pGenerate_PHPExcel->getSheet($i)->getChartCount() : 0;
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pGenerate_PHPExcel->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof Generate_PHPExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof Generate_PHPExcel_Worksheet_MemoryDrawing) {
			...
			if($pGenerate_PHPExcel->hasRibbonBinObjects()){//Some additional objects in the ribbon ?
				//we need to write "Extension" but not already write for media content
				$tabRibbonTypes=array_diff($pGenerate_PHPExcel->getRibbonBinObjects('types'), array_keys($aMediaContentTypes));
			...
			$sheetCount = $pGenerate_PHPExcel->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pGenerate_PHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($pGenerate_PHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (Generate_PHPExcel_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new Generate_PHPExcel_Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeDefaultContentType(Generate_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeOverrideContentType(Generate_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
