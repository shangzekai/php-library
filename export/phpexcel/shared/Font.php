...
/**
 * Generate_PHPExcel_Shared_Font
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_Shared
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Shared_Font
{
	 ...
	 *
	 * @param 	Generate_PHPExcel_Style_Font			$font			Font object
	 * @param 	Generate_PHPExcel_RichText|string	$cellText		Text to calculate width
	 * @param 	integer						$rotation		Rotation angle
	 * @param 	Generate_PHPExcel_Style_Font|NULL	$defaultFont	Font object
	 * @return 	integer		Column width
	 */
	public static function calculateColumnWidth(Generate_PHPExcel_Style_Font $font, $cellText = '', $rotation = 0, Generate_PHPExcel_Style_Font $defaultFont = null) {

		// If it is rich text, use plain text
		if ($cellText instanceof Generate_PHPExcel_RichText) {
		...
		try {
			...
			if (self::$autoSizeMethod == self::AUTOSIZE_METHOD_APPROX) {
				throw new Generate_PHPExcel_Exception('AutoSize method is set to approx');
			}
		...
		$columnWidth = Generate_PHPExcel_Shared_Drawing::pixelsToCellDimension($columnWidth, $defaultFont);
	 ...
	 *
	 * @param string $text
	 * @param Generate_PHPExcel_Style_Font
	 * @param int $rotation
	 * @return int
	 * @throws Generate_PHPExcel_Exception
	 */
	public static function getTextWidthPixelsExact($text, Generate_PHPExcel_Style_Font $font, $rotation = 0) {
		if (!function_exists('imagettfbbox')) {
			throw new Generate_PHPExcel_Exception('GD library needs to be enabled');
		}
	 ...
	 */
	public static function getTextWidthPixelsApprox($columnText, Generate_PHPExcel_Style_Font $font = null, $rotation = 0)
	{
		...
		switch ($fontName) {
			case 'Calibri':
				...
				$columnWidth = (int) (8.26 * Generate_PHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 11; // extrapolate from font size
				break;

			case 'Arial':
				// value 7 was found via interpolation by inspecting real Excel files with Arial 10 font.
				$columnWidth = (int) (7 * Generate_PHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;

			case 'Verdana':
				// value 8 was found via interpolation by inspecting real Excel files with Verdana 10 font.
				$columnWidth = (int) (8 * Generate_PHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;
	...
	public static function getTrueTypeFontFileFromFont($font) {
		if (!file_exists(self::$trueTypeFontPath) || !is_dir(self::$trueTypeFontPath)) {
			throw new Generate_PHPExcel_Exception('Valid directory to TrueType Font files not specified');
		}
		...
		switch ($name) {
			...
			default:
				throw new Generate_PHPExcel_Exception('Unknown font name "'. $name .'". Cannot map to TrueType font file');
				break;
		}
		...
		if (!file_exists($fontFile)) {
			throw New Generate_PHPExcel_Exception('TrueType Font file not found');
		}
	 ...
	 *
	 * @param Generate_PHPExcel_Style_Font $font The workbooks default font
	 * @param boolean $pPixels true = return column width in pixels, false = return in OOXML units
	 * @return mixed Column width
	 */
	public static function getDefaultColumnWidthByFont(Generate_PHPExcel_Style_Font $font, $pPixels = false)
	{
	 ...
	 */
	public static function getDefaultRowHeightByFont(Generate_PHPExcel_Style_Font $font)
	{
		switch ($font->getName()) {
			case 'Arial':
				switch ($font->getSize()) {
