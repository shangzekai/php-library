...
/**
 * Generate_PHPExcel_Style_NumberFormat
 *
 * @category   Generate_PHPExcel
 * @package	Generate_PHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Style_NumberFormat extends Generate_PHPExcel_Style_Supervisor implements Generate_PHPExcel_IComparable
{
	/* Pre-defined formats */
	 ...
	 * @var string
	 */
	protected $_formatCode	=	Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new Generate_PHPExcel_Style_NumberFormat
	 *
	 ...
	 *
	 * <code>
	 * $objGenerate_PHPExcel->getActiveSheet()->getStyle('B2')->getNumberFormat()->applyFromArray(
	 *		array(
	 *			'code' => Generate_PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return Generate_PHPExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$dateObj = Generate_PHPExcel_Shared_Date::ExcelToPHPObject($value);
	...
	private static function _formatAsFraction(&$value, &$format)
	{
		...
		$GCD = Generate_PHPExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
	 ...
	 */
	public static function toFormattedString($value = '0', $format = Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		if (($format === Generate_PHPExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === Generate_PHPExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return $value;
		}
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
										$value
										, strlen($right)
										, Generate_PHPExcel_Shared_String::getDecimalSeparator()
										, Generate_PHPExcel_Shared_String::getThousandsSeparator()
									);
