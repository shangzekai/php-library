...
/**
 * Generate_PHPExcel_Writer_Excel5
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Writer_Excel5 extends Generate_PHPExcel_Writer_Abstract implements Generate_PHPExcel_Writer_IWriter
{
	/**
	...
	/**
	 * Create a new Generate_PHPExcel_Writer_Excel5
	 *
	...
	public function __construct(Generate_PHPExcel $Generate_PHPExcel) {
		...
		$this->_parser		= new Generate_PHPExcel_Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$this->_writerWorkbook = new Generate_PHPExcel_Writer_Excel5_Workbook($this->_Generate_PHPExcel,
																	 $this->_str_total, $this->_str_unique, $this->_str_table,
																	 $this->_colors, $this->_parser);

		// Initialise worksheet writers
		$countSheets = $this->_Generate_PHPExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new Generate_PHPExcel_Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
																			   $this->_str_table, $this->_colors,
																			   $this->_parser,
																			   $this->_preCalculateFormulas,
																			   $this->_Generate_PHPExcel->getSheet($i));
		}
		...
		$cellXfCollection = $this->_Generate_PHPExcel->getCellXfCollection();
		for ($i = 0; $i < 15; ++$i) {
			$this->_writerWorkbook->addXfWriter($cellXfCollection[0], true);
		}

		// add all the cell Xfs
		foreach ($this->_Generate_PHPExcel->getCellXfCollection() as $style) {
		...
		$OLE = new Generate_PHPExcel_Shared_OLE_PPS_File(Generate_PHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new Generate_PHPExcel_Shared_OLE_PPS_File(Generate_PHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new Generate_PHPExcel_Shared_OLE_PPS_File(Generate_PHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new Generate_PHPExcel_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
	 ...
	 * @return Generate_PHPExcel_Writer_Excel5
	 */
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_Generate_PHPExcel->getAllsheets() as $sheet) {
			// sheet index
			...
			$escher = new Generate_PHPExcel_Shared_Escher();

			// dgContainer
			$dgContainer = new Generate_PHPExcel_Shared_Escher_DgContainer();
			...
			// spgrContainer
			$spgrContainer = new Generate_PHPExcel_Shared_Escher_DgContainer_SpgrContainer();
			$dgContainer->setSpgrContainer($spgrContainer);

			// add one shape which is the group shape
			$spContainer = new Generate_PHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = Generate_PHPExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = Generate_PHPExcel_Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new Generate_PHPExcel_Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = Generate_PHPExcel_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = Generate_PHPExcel_Cell::stringFromColumnIndex(Generate_PHPExcel_Cell::stringFromColumnIndex($iInc - 1));
	...
	private function _buildWorkbookEscher()
	{
		...
		// dggContainer
		$dggContainer = new Generate_PHPExcel_Shared_Escher_DggContainer();
			...
			if (count($sheet->getDrawingCollection()) > 0) {
				...
				foreach ($sheet->getDrawingCollection() as $drawing) {
					...
					$spId = $sheetCountShapes
						| ($this->_Generate_PHPExcel->getIndex($sheet) + 1) << 10;
		...
		// bstoreContainer
		$bstoreContainer = new Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof Generate_PHPExcel_Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(Generate_PHPExcel_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
					...
					$blip = new Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
					$blip->setData($blipData);

					$BSE = new Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE();
					$BSE->setBlipType($blipType);
					$BSE->setBlip($blip);

					$bstoreContainer->addBSE($BSE);

				} else if ($drawing instanceof Generate_PHPExcel_Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_GIF:
					case Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG:
					case Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeDocumentSummaryInformation(){
		...
		if($this->_Generate_PHPExcel->getProperties()->getCategory()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getCategory();
	...
	private function _writeSummaryInformation(){
		...
		//	Title
		if($this->_Generate_PHPExcel->getProperties()->getTitle()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getTitle();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x02),
								   'offset' => array('pack' => 'V'),
		...
		//	Subject
		if($this->_Generate_PHPExcel->getProperties()->getSubject()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getSubject();
		...
		//	Author (Creator)
		if($this->_Generate_PHPExcel->getProperties()->getCreator()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getCreator();
		...
		//	Keywords
		if($this->_Generate_PHPExcel->getProperties()->getKeywords()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getKeywords();
		...
		if($this->_Generate_PHPExcel->getProperties()->getDescription()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getDescription();
		...
		if($this->_Generate_PHPExcel->getProperties()->getLastModifiedBy()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getLastModifiedBy();
		...
		//	Created Date/Time
		if($this->_Generate_PHPExcel->getProperties()->getCreated()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getCreated();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   'offset' => array('pack' => 'V'),
								   ...
								   'data'	=> array('data' => Generate_PHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
		if($this->_Generate_PHPExcel->getProperties()->getModified()){
			$dataProp = $this->_Generate_PHPExcel->getProperties()->getModified();
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0D),
								   'offset' => array('pack' => 'V'),
								   'type' 	=> array('pack' => 'V', 'data' => 0x40), // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
								   'data'	=> array('data' => Generate_PHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
