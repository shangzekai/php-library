...
/**
 * Generate_PHPExcel_Worksheet_AutoFilter_Column_Rule
 *
 * @category	Generate_PHPExcel
 * @package		Generate_PHPExcel_Worksheet
 * @copyright	Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Worksheet_AutoFilter_Column_Rule
{
...
//	const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER	= 'allDatesInQuarter';	//	<dynamicFilter type="Q2"/> for Quarter 2
	 ...
	 * Autofilter Column
	 *
	 * @var Generate_PHPExcel_Worksheet_AutoFilter_Column
	 */
	...
	/**
	 * Create a new Generate_PHPExcel_Worksheet_AutoFilter_Column_Rule
	 *
	 * @param Generate_PHPExcel_Worksheet_AutoFilter_Column $pParent
	 */
	public function __construct(Generate_PHPExcel_Worksheet_AutoFilter_Column $pParent = NULL)
	{
	...
	public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER) {
		if (!in_array($pRuleType,self::$_ruleTypes)) {
			throw new Generate_PHPExcel_Exception('Invalid rule type for column AutoFilter Rule.');
		}
	...
	public function setValue($pValue = '') {
		if (is_array($pValue)) {
			...
			if (count($pValue) == 0) {
				throw new Generate_PHPExcel_Exception('Invalid rule value for column AutoFilter Rule.');
			}
	...
	public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL) {
		...
		if ((!in_array($pOperator,self::$_operators)) &&
			...
			throw new Generate_PHPExcel_Exception('Invalid operator for column AutoFilter Rule.');
		}
	 ...
	 *
	 * @param Generate_PHPExcel_Worksheet_AutoFilter_Column
	 * @return Generate_PHPExcel_Worksheet_AutoFilter_Column_Rule
	 */
	public function setParent(Generate_PHPExcel_Worksheet_AutoFilter_Column $pParent = NULL) {
		$this->_parent = $pParent;
