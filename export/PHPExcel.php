...
if (!defined('Generate_PHPExcel_ROOT')) {
    ...
    require(Generate_PHPExcel_ROOT . 'phpexcel/Autoloader.php');
}
...
class Generate_PHPExcel
{
     ...
     * Document properties
     *
     * @var Generate_PHPExcel_DocumentProperties
     */
    private $_properties;
     ...
     * Document security
     *
     * @var Generate_PHPExcel_DocumentSecurity
     */
    private $_security;
     ...
     * Named ranges
     *
     * @var Generate_PHPExcel_NamedRange[]
     */
	...
	*/
	...
	public function getSheetByCodeName($pName = '')
	{
	 ...
	 /**
	 * Create a new Generate_PHPExcel with one Worksheet
	 */
	public function __construct()
	{
		$this->_uniqueID = uniqid();
		$this->_calculationEngine	= Generate_PHPExcel_Calculation::getInstance($this);

		// Initialise worksheet collection and add one worksheet
		$this->_workSheetCollection = array();
		$this->_workSheetCollection[] = new Generate_PHPExcel_Worksheet($this);
		$this->_activeSheetIndex = 0;

        // Create document properties
        $this->_properties = new Generate_PHPExcel_DocumentProperties();

        // Create document security
        $this->_security = new Generate_PHPExcel_DocumentSecurity();

        // Set named ranges
        $this->_namedRanges = array();

        // Create the cellXf supervisor
        $this->_cellXfSupervisor = new Generate_PHPExcel_Style(true);
        $this->_cellXfSupervisor->bindParent($this);

        // Create the default style
        $this->addCellXf(new Generate_PHPExcel_Style);
        $this->addCellStyleXf(new Generate_PHPExcel_Style);
    }
    ...
    public function __destruct() {
        Generate_PHPExcel_Calculation::unsetInstance($this);
        $this->disconnectWorksheets();
    }    //    function __destruct()

    /**
     * Disconnect all worksheets from this Generate_PHPExcel workbook object,
     *    typically so that the Generate_PHPExcel object can be unset
     *
     */
     ...
     * Get properties
     *
     * @return Generate_PHPExcel_DocumentProperties
     */
     ...
     * Set properties
     *
     * @param Generate_PHPExcel_DocumentProperties    $pValue
     */
    public function setProperties(Generate_PHPExcel_DocumentProperties $pValue)
    {
     ...
     * Get security
     *
     * @return Generate_PHPExcel_DocumentSecurity
     */
     ...
     * Set security
     *
     * @param Generate_PHPExcel_DocumentSecurity    $pValue
     */
    public function setSecurity(Generate_PHPExcel_DocumentSecurity $pValue)
    {
    ...
    {
        $newSheet = new Generate_PHPExcel_Worksheet($this);
     ...
     * @throws Generate_PHPExcel_Exception
     */
    public function addSheet(Generate_PHPExcel_Worksheet $pSheet, $iSheetIndex = NULL)
    {
        if ($this->sheetNameExists($pSheet->getTitle())) {
            throw new Generate_PHPExcel_Exception(
     ...
     * @throws Generate_PHPExcel_Exception
     */
    public function getIndex(Generate_PHPExcel_Worksheet $pSheet)
    {
        ...
        throw new Generate_PHPExcel_Exception("Sheet does not exist.");
    }
    ...
    {
        if (($worksheet = $this->getSheetByName($pValue)) instanceof Generate_PHPExcel_Worksheet) {
            $this->setActiveSheetIndex($this->getIndex($worksheet));
            return $worksheet;
        }

        throw new Generate_PHPExcel_Exception('Workbook does not contain sheet:' . $pValue);
    }
     ...
     * Add external sheet
     *
     * @param  Generate_PHPExcel_Worksheet $pSheet External sheet to add
     * @param  int|null $iSheetIndex Index where sheet should go (0,1,..., or null for last)
     * @throws Generate_PHPExcel_Exception
     * @return Generate_PHPExcel_Worksheet
     */
    public function addExternalSheet(Generate_PHPExcel_Worksheet $pSheet, $iSheetIndex = null) {
        if ($this->sheetNameExists($pSheet->getTitle())) {
            throw new Generate_PHPExcel_Exception("Workbook already contains a worksheet named '{$pSheet->getTitle()}'. Rename the external sheet first.");
        }
     ...
     * Get named ranges
     *
     * @return Generate_PHPExcel_NamedRange[]
     */
     ...
     * Add named range
     *
     * @param  Generate_PHPExcel_NamedRange $namedRange
     * @return Generate_PHPExcel
     */
    public function addNamedRange(Generate_PHPExcel_NamedRange $namedRange) {
     ...
     * @param  Generate_PHPExcel_Worksheet|null $pSheet Scope. Use null for global scope
     * @return Generate_PHPExcel_NamedRange|null
     */
    public function getNamedRange($namedRange, Generate_PHPExcel_Worksheet $pSheet = null) {
        $returnValue = null;
     ...
     * @return Generate_PHPExcel
     */
    public function removeNamedRange($namedRange, Generate_PHPExcel_Worksheet $pSheet = null) {
        if ($pSheet === NULL) {
            if (isset($this->_namedRanges[$namedRange])) {
     ...
     *
     * @return Generate_PHPExcel_WorksheetIterator
     */
    public function getWorksheetIterator() {
        return new Generate_PHPExcel_WorksheetIterator($this);
    }
     ...
     *
     * @param  string $pValue
     * @return Generate_PHPExcel_Style|false
     */
     ...
     *
     * @param  Generate_PHPExcel_Style $pCellStyle
     * @return boolean
     */
    ...
    public function getDefaultStyle()
    {
        ...
        throw new Generate_PHPExcel_Exception('No default style found for this workbook');
    }

    /**
     * Add a cellXf to the workbook
     *
     * @param Generate_PHPExcel_Style $style
     */
    public function addCellXf(Generate_PHPExcel_Style $style)
    {
    ...
    public function removeCellXfByIndex($pIndex = 0)
    {
        if ($pIndex > count($this->_cellXfCollection) - 1) {
            throw new Generate_PHPExcel_Exception("CellXf index is out of bounds.");
        } else {
     ...
     *
     * @param Generate_PHPExcel_Style $pStyle
     */
    public function addCellStyleXf(Generate_PHPExcel_Style $pStyle)
    {
    ...
    public function removeCellStyleXfByIndex($pIndex = 0)
    {
        if ($pIndex > count($this->_cellStyleXfCollection) - 1) {
            throw new Generate_PHPExcel_Exception("CellStyleXf index is out of bounds.");
        } else {
    ...
    public function garbageCollect()
    {
        ...
        if (empty($this->_cellXfCollection)) {
            $this->_cellXfCollection[] = new Generate_PHPExcel_Style();
        }
