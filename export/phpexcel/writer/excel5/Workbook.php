...
/**
 * Generate_PHPExcel_Writer_Excel5_Workbook
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Writer_Excel5_Workbook extends Generate_PHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	...
	/**
	 * XF Writers
	 * @var Generate_PHPExcel_Writer_Excel5_Xf[]
	 */
	 ...
	 * Fonts writers
	 *
	 * @var Generate_PHPExcel_Writer_Excel5_Font[]
	 */
	 ...
	 * Class constructor
	 *
	 * @param Generate_PHPExcel	$Generate_PHPExcel		The Workbook
	 ...
	 */
	public function __construct(Generate_PHPExcel $Generate_PHPExcel = null,
								...
								$parser )
	{
		...
		$countSheets = $Generate_PHPExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$phpSheet = $Generate_PHPExcel->getSheet($i);
	 ...
	 *
	 * @param Generate_PHPExcel_Style_Font $font
	 * @return int Index to FONT record
	 */
	public function _addFont(Generate_PHPExcel_Style_Font $font)
	{
		...
		if(isset($this->_addedFonts[$fontHashCode])){
			...
			$fontWriter = new Generate_PHPExcel_Writer_Excel5_Font($font);
	...
	public function writeWorkbook($pWorksheetSizes = null)
	{
		...
		$total_worksheets = $this->_Generate_PHPExcel->getSheetCount();
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$this->_writeBoundsheet($this->_Generate_PHPExcel->getSheet($i), $this->_worksheetOffsets[$i]);
		}
	...
	function _calcSheetOffsets()
	{
		...
		$total_worksheets = count($this->_Generate_PHPExcel->getAllSheets());
		foreach ($this->_Generate_PHPExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + strlen(Generate_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeExterns()
	{
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writeExternsheet($this->_Generate_PHPExcel->getSheet($i)->getTitle());
		}
	}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$sheetSetup = $this->_Generate_PHPExcel->getSheet($i)->getPageSetup();
			// Write a Name record if the print area has been defined
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = Generate_PHPExcel_Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = Generate_PHPExcel_Cell::coordinateFromString($printArea[0]);
				$printArea[1] = Generate_PHPExcel_Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = Generate_PHPExcel_Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = Generate_PHPExcel_Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = Generate_PHPExcel_Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = Generate_PHPExcel_Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		// Named ranges
		if (count($this->_Generate_PHPExcel->getNamedRanges()) > 0) {
			// Loop named ranges
			$namedRanges = $this->_Generate_PHPExcel->getNamedRanges();
			foreach ($namedRanges as $namedRange) {

				// Create absolute coordinate
				$range = Generate_PHPExcel_Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . Generate_PHPExcel_Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = Generate_PHPExcel_Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = Generate_PHPExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
				...
				try {
					...
					if ($namedRange->getLocalOnly()) {
						// local scope
						$scope = $this->_Generate_PHPExcel->getIndex($namedRange->getScope()) + 1;
					} else {
						// global scope
						$scope = 0;
					}
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = Generate_PHPExcel_Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = Generate_PHPExcel_Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = Generate_PHPExcel_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = Generate_PHPExcel_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			$sheetAutoFilter = $this->_Generate_PHPExcel->getSheet($i)->getAutoFilter();
			$autoFilterRange = $sheetAutoFilter->getRange();
			if(!empty($autoFilterRange)) {
				$rangeBounds = Generate_PHPExcel_Cell::rangeBoundaries($autoFilterRange);
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = Generate_PHPExcel_Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(Generate_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
	...
	private function _writeWindow1()
	{
		...
		// not supported by Generate_PHPExcel, so there is only one selected sheet, the active
		$ctabsel   = 1;       // Number of workbook tabs selected

		$wTabRatio = 0x0258;                 // Tab to scrollbar ratio

		// not supported by Generate_PHPExcel, set to 0
		$itabFirst = 0;     // 1st displayed worksheet
		$itabCur   = $this->_Generate_PHPExcel->getActiveSheetIndex();    // Active worksheet
	 ...
	 *
	 * @param Generate_PHPExcel_Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case Generate_PHPExcel_Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case Generate_PHPExcel_Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case Generate_PHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		$data .= Generate_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
	...
	private function _writeSupbookInternal()
	{
		...
		$header    = pack("vv", $record, $length);
		$data      = pack("vv", $this->_Generate_PHPExcel->getSheetCount(), 0x0401);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		$numberFormatString = Generate_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (Generate_PHPExcel_Shared_Date::getExcelCalendar() == Generate_PHPExcel_Shared_Date::CALENDAR_MAC_1904) ?
