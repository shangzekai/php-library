...
/**
 * Generate_PHPExcel_CachedObjectStorage_Wincache
 *
 * @category   Generate_PHPExcel
 * @package	Generate_PHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_CachedObjectStorage_Wincache extends Generate_PHPExcel_CachedObjectStorage_CacheBase implements Generate_PHPExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty && !empty($this->_currentObjectID)) {
			...
			if (wincache_ucache_exists($this->_cachePrefix.$this->_currentObjectID.'.cache')) {
				if (!wincache_ucache_set($this->_cachePrefix.$this->_currentObjectID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new Generate_PHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in WinCache');
				}
			} else {
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new Generate_PHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in WinCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($success === false) {
					...
					throw new Generate_PHPExcel_Exception('Cell entry '.$cellID.' no longer exists in Wincache');
				}
				if (!wincache_ucache_add($newCachePrefix.$cellID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new Generate_PHPExcel_Exception('Failed to store cell '.$cellID.' in Wincache');
				}
			}
		}
