...
/**
 * Generate_PHPExcel_Writer_Excel2007_Chart
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Writer_Excel2007_Chart extends Generate_PHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeChart(Generate_PHPExcel_Chart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * Write Chart Title
	 *
	 * @param	Generate_PHPExcel_Chart_Title		$title
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeTitle(Generate_PHPExcel_Chart_Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * Write Chart Legend
	 *
	 * @param	Generate_PHPExcel_Chart_Legend		$legend
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeLegend(Generate_PHPExcel_Chart_Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 *
	 * @param	Generate_PHPExcel_Chart_PlotArea		$plotArea
	 * @param	Generate_PHPExcel_Chart_Title		$xAxisLabel
	 * @param	Generate_PHPExcel_Chart_Title		$yAxisLabel
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writePlotArea(Generate_PHPExcel_Chart_PlotArea $plotArea,
									Generate_PHPExcel_Chart_Title $xAxisLabel = NULL,
									Generate_PHPExcel_Chart_Title $yAxisLabel = NULL,
									$objWriter,
									Generate_PHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotArea)) {
			return;
		}
				...
				$objWriter->startElement('c:'.$chartType);
					...
					for($i = 0; $i < $groupCount; ++$i) {
						...
						if ($groupType == $chartType) {
							...
							if ($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_RADARCHART) {
								$objWriter->startElement('c:radarStyle');
									$objWriter->writeAttribute('val', $plotStyle );
								$objWriter->endElement();
							} elseif ($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_SCATTERCHART) {
					...
					if ($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
						//	Line only, Line3D can't be smoothed

						$objWriter->startElement('c:smooth');
							$objWriter->writeAttribute('val', (integer) $plotGroup->getSmoothLine() );
						$objWriter->endElement();
					} elseif (($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
						($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
						...
						if ($plotGroupingType == 'percentStacked' ||
					...
					} elseif ($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
							...
							$objWriter->startElement('c:showNegBubbles');
								$objWriter->writeAttribute('val', 0 );
							$objWriter->endElement();
					} elseif ($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_STOCKCHART) {
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== Generate_PHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
						($chartType !== Generate_PHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== Generate_PHPExcel_Chart_DataSeries::TYPE_DONUTCHART)) {
						...
						if ($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_DONUTCHART) {
			...
			if (($chartType !== Generate_PHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
				...
				if ($chartType === Generate_PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	 ...
	 *
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @param 	Generate_PHPExcel_Chart_Layout		$chartLayout	Chart layout
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	 ...
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeCatAx($objWriter, Generate_PHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(Generate_PHPExcel_Shared_String::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeValAx($objWriter, Generate_PHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
					...
					if ($groupType !== Generate_PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	...
	private static function _getChartType($plotArea)
	{
		...
		if ($groupCount == 1) {
			...
			if (count($chartTypes) == 0) {
				throw new Generate_PHPExcel_Writer_Exception('Chart is not yet implemented');
			}
		}
	 ...
	 *
	 * @param	Generate_PHPExcel_Chart_DataSeries		$plotGroup
									...
									)
	{
		...
		if (($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
			($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
		...
		if (($groupType !== Generate_PHPExcel_Chart_DataSeries::TYPE_RADARCHART) &&
			($groupType !== Generate_PHPExcel_Chart_DataSeries::TYPE_STOCKCHART)) {

			if ($groupType !== Generate_PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
				if (($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_PIECHART) ||
					($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) ||
					($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_DONUTCHART) ||
					($plotSeriesCount > 1)) {
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			$objWriter->startElement('c:ser');
				...
				if (($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_LINECHART) ||
					...
					$objWriter->startElement('c:spPr');
						$objWriter->startElement('a:ln');
            				...
            				if ($groupType == Generate_PHPExcel_Chart_DataSeries::TYPE_STOCKCHART) {
				...
				if (($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
					($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D) ||
					($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART)) {
				...
				if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
					...
					if (($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) ||
						($groupType === Generate_PHPExcel_Chart_DataSeries::TYPE_SCATTERCHART)) {
	 ...
	 *
	 * @param	Generate_PHPExcel_Chart_DataSeriesValues		$plotSeriesLabel
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	 ...
	 *
	 * @param	Generate_PHPExcel_Chart_DataSeriesValues		$plotSeriesValues
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @param	string								$groupType			Type of plot for dataseries
	 ...
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, Generate_PHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * Write Layout
	 *
	 * @param	Generate_PHPExcel_Chart_Layout		$layout
	 * @param 	Generate_PHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	Generate_PHPExcel_Writer_Exception
	 */
	private function _writeLayout(Generate_PHPExcel_Chart_Layout $layout = NULL, $objWriter)
	{
