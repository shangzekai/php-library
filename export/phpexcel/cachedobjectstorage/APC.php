...
/**
 * Generate_PHPExcel_CachedObjectStorage_APC
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_CachedObjectStorage_APC extends Generate_PHPExcel_CachedObjectStorage_CacheBase implements Generate_PHPExcel_CachedObjectStorage_ICache {
    ...
    protected function _storeData() {
        if ($this->_currentCellIsDirty && !empty($this->_currentObjectID)) {
            ...
            if (!apc_store($this->_cachePrefix.$this->_currentObjectID.'.cache',serialize($this->_currentObject),$this->_cacheTime)) {
                $this->__destruct();
                throw new Generate_PHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in APC');
            }
    ...
    public function isDataSet($pCoord) {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === FALSE) {
                ...
                throw new Generate_PHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in APC cache');
            }
            return true;
        }
        return false;
        ...
        foreach($cacheList as $cellID) {
            if ($cellID != $this->_currentObjectID) {
                ...
                if ($obj === FALSE) {
                    ...
                    throw new Generate_PHPExcel_Exception('Cell entry '.$cellID.' no longer exists in APC');
                }
                if (!apc_store($newCachePrefix.$cellID.'.cache',$obj,$this->_cacheTime)) {
                    $this->__destruct();
                    throw new Generate_PHPExcel_Exception('Failed to store cell '.$cellID.' in APC');
                }
            }
        }
