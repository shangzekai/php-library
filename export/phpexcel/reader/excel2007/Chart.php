 ...
 * @package		Generate_PHPExcel_Reader_Excel2007
...
/**
 * Generate_PHPExcel_Reader_Excel2007_Chart
 *
 * @category	Generate_PHPExcel
 * @package		Generate_PHPExcel_Reader_Excel2007
 * @copyright	Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Reader_Excel2007_Chart
{
						...
						switch ($chartDetailsKey) {
							case "plotArea":
									...
									if ($plotAreaLayout == NULL) {
										$plotAreaLayout = new Generate_PHPExcel_Chart_Layout();
									}
									$plotArea = new Generate_PHPExcel_Chart_PlotArea($plotAreaLayout,$plotSeries);
							...
							case "legend":
									...
									$legend = new Generate_PHPExcel_Chart_Legend($legendPos, $legendLayout, $legendOverlay);
									break;
						}
					}
			}
		}
		$chart = new Generate_PHPExcel_Chart($chartName,$title,$legend,$plotArea,$plotVisOnly,$dispBlanksAs,$XaxisLabel,$YaxisLabel);
	...
	private static function _chartTitle($titleDetails,$namespacesChartMeta,$type) {
		...
		return new Generate_PHPExcel_Chart_Title($caption, $titleLayout);
...
//			echo $detailKey,' => ',self::_getAttribute($detail, 'val', 'string'),PHP_EOL;
		...
		return new Generate_PHPExcel_Chart_Layout($layout);
	...
	private static function _chartDataSeries($chartDetail,$namespacesChartMeta,$plotType) {
		...
		return new Generate_PHPExcel_Chart_DataSeries($plotType,$multiSeriesType,$plotOrder,$seriesLabel,$seriesCategory,$seriesValues,$smoothLine);
	...
	private static function _chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta, $marker = null, $smoothLine = false) {
		if (isset($seriesDetail->strRef)) {
			...
			return new Generate_PHPExcel_Chart_DataSeriesValues('String',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		} elseif (isset($seriesDetail->numRef)) {
			$seriesSource = (string) $seriesDetail->numRef->f;
			$seriesData = self::_chartDataSeriesValues($seriesDetail->numRef->numCache->children($namespacesChartMeta['c']));

			return new Generate_PHPExcel_Chart_DataSeriesValues('Number',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		...
		foreach($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
			...
			if (isset($titleDetailElement->rPr)) {
				...
				if (!is_null($fontColor)) {
					$objText->getFont()->setColor( new Generate_PHPExcel_Style_Color( self::_readColor($fontColor) ) );
				}
				...
				if (!is_null($underscore)) {
					if ($underscore == 'sng') {
						...
						$objText->getFont()->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_DOUBLE);
					} else {
						$objText->getFont()->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_NONE);
					}
				}
