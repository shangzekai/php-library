 ...
 * @package    Generate_PHPExcel_Reader_Excel5
...
// http://sourceforge.net/projects/Generate_PHPExcelreader/
...
/**
 * Generate_PHPExcel_Reader_Excel5
 *
 * This class uses {@link http://sourceforge.net/projects/Generate_PHPExcelreader/parseXL}
 *
 * @category	Generate_PHPExcel
 * @package		Generate_PHPExcel_Reader_Excel5
 * @copyright	Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_Reader_Excel5 extends Generate_PHPExcel_Reader_Abstract implements Generate_PHPExcel_Reader_IReader
{
	// ParseXL definitions
	 ...
	 *
	 * @var Generate_PHPExcel_Reader_Excel5_RC4
	 */
	...
	/**
	 * Create a new Generate_PHPExcel_Reader_Excel5 instance
	 */
	public function __construct() {
		$this->_readFilter = new Generate_PHPExcel_Reader_DefaultReadFilter();
	}


	/**
	 * Can the current Generate_PHPExcel_Reader_IReader read the file?
	 *
	 * @param 	string 		$pFilename
	 * @return 	boolean
	 * @throws Generate_PHPExcel_Reader_Exception
	 */
	public function canRead($pFilename)
	{
		// Check if file exists
		if (!file_exists($pFilename)) {
			throw new Generate_PHPExcel_Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
		}

		try {
			// Use ParseXL for the hard work.
			$ole = new Generate_PHPExcel_Shared_OLERead();
	...
	/**
	 * Reads names of the worksheets from a file, without parsing the whole file to a Generate_PHPExcel object
	 *
	 * @param 	string 		$pFilename
	 * @throws 	Generate_PHPExcel_Reader_Exception
	 */
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			$tmpInfo['lastColumnLetter'] = Generate_PHPExcel_Cell::stringFromColumnIndex($tmpInfo['lastColumnIndex']);
	...
	/**
	 * Loads Generate_PHPExcel from file
	 *
	 * @param 	string 		$pFilename
	...
	public function load($pFilename)
	{
		...
		// Initialisations
		$this->_Generate_PHPExcel = new Generate_PHPExcel;
		$this->_Generate_PHPExcel->removeSheetByIndex(0); // remove 1st sheet
		if (!$this->_readDataOnly) {
			$this->_Generate_PHPExcel->removeCellStyleXfByIndex(0); // remove the default style
			$this->_Generate_PHPExcel->removeCellXfByIndex(0); // remove the default style
		}
		...
		if (!$this->_readDataOnly) {
			...
			foreach ($this->_Generate_PHPExcel->getCellXfCollection() as $objStyle) {
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new Generate_PHPExcel_Shared_Escher();
			$reader = new Generate_PHPExcel_Reader_Excel5_Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new Generate_PHPExcel_Shared_Escher());
		...
		foreach ($this->_sheets as $sheet) {
			...
			// add sheet to Generate_PHPExcel object
			$this->_phpSheet = $this->_Generate_PHPExcel->createSheet();
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorksheet = new Generate_PHPExcel_Shared_Escher();
				$reader = new Generate_PHPExcel_Reader_Excel5_Escher($escherWorksheet);
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					list($startColumn, $startRow) = Generate_PHPExcel_Cell::coordinateFromString($spContainer->getStartCoordinates());
					list($endColumn, $endRow) = Generate_PHPExcel_Cell::coordinateFromString($spContainer->getEndCoordinates());
					...
					$width = Generate_PHPExcel_Shared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
					$height = Generate_PHPExcel_Shared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

					// calculate offsetX and offsetY of the shape
					$offsetX = $startOffsetX * Generate_PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
					$offsetY = $startOffsetY * Generate_PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
...
//							echo 'Picture Object<br />';
							...
							if ($blip = $BSE->getBlip()) {
								...
								$drawing = new Generate_PHPExcel_Worksheet_MemoryDrawing();
								...
								switch ($blipType) {
									case Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
										$drawing->setRenderingFunction(Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG);
										$drawing->setMimeType(Generate_PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
										break;

									case Generate_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
										$drawing->setRenderingFunction(Generate_PHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG);
										$drawing->setMimeType(Generate_PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_PNG);
										break;
								}
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					list($column, $row) = Generate_PHPExcel_Cell::coordinateFromString($cell);
					if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, Generate_PHPExcel_Cell_DataType::TYPE_FORMULA);
					}
				}
			}
...
//					echo '<br />';
		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					if ($docSheet = $this->_Generate_PHPExcel->getSheetByName($sheetName)) {
				...
				case pack('C', 0x07):
					...
					foreach ($ranges as $range) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_Generate_PHPExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = Generate_PHPExcel_Cell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = Generate_PHPExcel_Cell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2) {
					if (($docSheet = $this->_Generate_PHPExcel->getSheetByName($explodes[0])) ||
						($docSheet = $this->_Generate_PHPExcel->getSheetByName(trim($explodes[0],"'")))) {
						$extractedRange = $explodes[1];
						$extractedRange = str_replace('$', '', $extractedRange);
						...
						$scope = ($definedName['scope'] == 0) ?
							null : $this->_Generate_PHPExcel->getSheetByName($this->_sheets[$definedName['scope'] - 1]['name']);

						$this->_Generate_PHPExcel->addNamedRange( new Generate_PHPExcel_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope) );
					}
				} else {
					//	Named Value
		...
		return $this->_Generate_PHPExcel;
	}
	...
	private function _readRecordData($data, $pos, $len)
	{
		...
		if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
			...
			throw new Generate_PHPExcel_Reader_Exception('XOr encryption not supported');
		}
		return $recordData;
	}
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = Generate_PHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;

				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = Generate_PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				case 0x01:	//	Code Page
					$codePage = Generate_PHPExcel_Shared_CodePage::NumberToName($value);
					break;

				case 0x02:	//	Title
					$this->_Generate_PHPExcel->getProperties()->setTitle($value);
					break;

				case 0x03:	//	Subject
					$this->_Generate_PHPExcel->getProperties()->setSubject($value);
					break;

				case 0x04:	//	Author (Creator)
					$this->_Generate_PHPExcel->getProperties()->setCreator($value);
					break;

				case 0x05:	//	Keywords
					$this->_Generate_PHPExcel->getProperties()->setKeywords($value);
					break;

				case 0x06:	//	Comments (Description)
					$this->_Generate_PHPExcel->getProperties()->setDescription($value);
					break;

				case 0x07:	//	Template
					//	Not supported by Generate_PHPExcel
					break;

				case 0x08:	//	Last Saved By (LastModifiedBy)
					$this->_Generate_PHPExcel->getProperties()->setLastModifiedBy($value);
					break;
				...
				case 0x0C:	//	Created Date/Time
					$this->_Generate_PHPExcel->getProperties()->setCreated($value);
					break;

				case 0x0D:	//	Modified Date/Time
					$this->_Generate_PHPExcel->getProperties()->setModified($value);
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = Generate_PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				...
				case 0x02:	//	Category
					$this->_Generate_PHPExcel->getProperties()->setCategory($value);
					break;
				...
				case 0x0E:	//	Manager
					$this->_Generate_PHPExcel->getProperties()->setManager($value);
					break;

				case 0x0F:	//	Company
					$this->_Generate_PHPExcel->getProperties()->setCompany($value);
					break;
...
//		echo '<br />';
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
					throw new Generate_PHPExcel_Reader_Exception('Cannot read this Excel file. Version is too old.');
				}
	...
	private function _readFilepass()
	{
		...
		if ($length != 54) {
			throw new Generate_PHPExcel_Reader_Exception('Unexpected file pass record length');
		}
		...
		if (!$this->_verifyPassword(
			...
			throw new Generate_PHPExcel_Reader_Exception('Decryption password incorrect');
		}
	 ...
	 * 
	 * @return Generate_PHPExcel_Reader_Excel5_RC4
	 */
	private function _makeKey($block, $valContext)
	{
		...
		$md5 = new Generate_PHPExcel_Reader_Excel5_MD5();
		$md5->add($pwarray);

		$s = $md5->getContext();
		return new Generate_PHPExcel_Reader_Excel5_RC4($s);
	}
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = Generate_PHPExcel_Shared_CodePage::NumberToName($codepage);
	}
	...
	private function _readDateMode()
	{
		...
		Generate_PHPExcel_Shared_Date::setExcelCalendar(Generate_PHPExcel_Shared_Date::CALENDAR_WINDOWS_1900);
		if (ord($recordData{0}) == 1) {
			Generate_PHPExcel_Shared_Date::setExcelCalendar(Generate_PHPExcel_Shared_Date::CALENDAR_MAC_1904);
		}
	}
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			$objFont = new Generate_PHPExcel_Style_Font();
			...
			switch ($underlineType) {
				case 0x00:
					break; // no underline
				case 0x01:
					$objFont->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_SINGLE);
					break;
				case 0x02:
					$objFont->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_DOUBLE);
					break;
				case 0x21:
					$objFont->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING);
					break;
				case 0x22:
					$objFont->setUnderline(Generate_PHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
					break;
			}
	...
	private function _readXf()
	{
		...
		$objStyle = new Generate_PHPExcel_Style();
		...
		if (!$this->_readDataOnly) {
			...
			} elseif (($code = Generate_PHPExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
			...
			$objStyle->getProtection()->setLocked($isLocked ?
				Generate_PHPExcel_Style_Protection::PROTECTION_INHERIT : Generate_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);

			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$objStyle->getProtection()->setHidden($isHidden ?
				Generate_PHPExcel_Style_Protection::PROTECTION_PROTECTED : Generate_PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
			...
			switch ($horAlign) {
				case 0:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_GENERAL);
					break;
				case 1:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 2:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 3:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 4:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_FILL);
					break;
				case 5:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				case 6:
					$objStyle->getAlignment()->setHorizontal(Generate_PHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
					break;
			}
			...
			switch ($vertAlign) {
				case 0:
					$objStyle->getAlignment()->setVertical(Generate_PHPExcel_Style_Alignment::VERTICAL_TOP);
					break;
				case 1:
					$objStyle->getAlignment()->setVertical(Generate_PHPExcel_Style_Alignment::VERTICAL_CENTER);
					break;
				case 2:
					$objStyle->getAlignment()->setVertical(Generate_PHPExcel_Style_Alignment::VERTICAL_BOTTOM);
					break;
				case 3:
					$objStyle->getAlignment()->setVertical(Generate_PHPExcel_Style_Alignment::VERTICAL_JUSTIFY);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell border lines and background area
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(Generate_PHPExcel_Style_Borders::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(Generate_PHPExcel_Style_Borders::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(Generate_PHPExcel_Style_Borders::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(Generate_PHPExcel_Style_Borders::DIAGONAL_BOTH);
					}
			...
			if ($isCellStyleXf) {
				...
				if ($this->_xfIndex == 0) {
					$this->_Generate_PHPExcel->addCellStyleXf($objStyle);
					$this->_mapCellStyleXfIndex[$this->_xfIndex] = 0;
				}
			} else {
				// we read all cell XF records
				$this->_Generate_PHPExcel->addCellXf($objStyle);
				$this->_mapCellXfIndex[$this->_xfIndex] = count($this->_Generate_PHPExcel->getCellXfCollection()) - 1;
			}
				...
				switch ($extType) {
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$fill = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFill();
					...
					case 7:		// border color top
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$top = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getTop();
					...
					case 8:		// border color bottom
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$bottom = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getBottom();
					...
					case 9:		// border color left
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$left = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getLeft();
					...
					case 10:		// border color right
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$right = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getRight();
					...
					case 11:		// border color diagonal
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$diagonal = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders()->getDiagonal();
					...
					case 13:	// font color
						...
						if ($xclfType == 2) {
							...
							// modify the relevant style property
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								$font = $this->_Generate_PHPExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getFont();
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isBuiltIn) {
				...
				// user-defined; not supported by Generate_PHPExcel
			}
		}
	}
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = Generate_PHPExcel_Worksheet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = Generate_PHPExcel_Worksheet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = Generate_PHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = Generate_PHPExcel_Worksheet::SHEETSTATE_VISIBLE;      break;
		}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, Generate_PHPExcel_Worksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, Generate_PHPExcel_Worksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(Generate_PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(Generate_PHPExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
	...
	private function _readRk()
	{
		...
		$columnString = Generate_PHPExcel_Cell::stringFromColumnIndex($column);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell
			$cell->setValueExplicit($numValue, Generate_PHPExcel_Cell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new Generate_PHPExcel_RichText();
				$charPos = 0;
				$sstCount = count($this->_sst[$index]['fmtRuns']);
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = Generate_PHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = Generate_PHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, Generate_PHPExcel_Shared_String::CountCharacters($this->_sst[$index]['value']));
					}

					if (Generate_PHPExcel_Shared_String::CountCharacters($text) > 0) {
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($richText, Generate_PHPExcel_Cell_DataType::TYPE_STRING);
			} else {
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], Generate_PHPExcel_Cell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = Generate_PHPExcel_Cell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readFormula()
	{
		...
		if ($isPartOfSharedFormula) {
			...
			$this->_baseCell = Generate_PHPExcel_Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
		}
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = Generate_PHPExcel_Cell_DataType::TYPE_STRING;
				...
				$dataType = Generate_PHPExcel_Cell_DataType::TYPE_BOOL;
				...
				$dataType = Generate_PHPExcel_Cell_DataType::TYPE_ERROR;
				...
				$dataType = Generate_PHPExcel_Cell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = Generate_PHPExcel_Cell_DataType::TYPE_NUMERIC;
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					if ($this->_version != self::XLS_BIFF8) {
						throw new Generate_PHPExcel_Reader_Exception('Not BIFF8. Can only read BIFF8 formulas');
					}
					$formula = $this->_getFormulaFromStructure($formulaStructure); // get formula in human language
					$cell->setValueExplicit('=' . $formula, Generate_PHPExcel_Cell_DataType::TYPE_FORMULA);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			switch ($isError) {
				case 0: // boolean
					...
					// add cell value
					$cell->setValueExplicit($value, Generate_PHPExcel_Cell_DataType::TYPE_BOOL);
					break;
				...
				case 1: // error type
					...
					// add cell value
					$cell->setValueExplicit($value, Generate_PHPExcel_Cell_DataType::TYPE_ERROR);
					break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = Generate_PHPExcel_Cell::stringFromColumnIndex($fc + $i);
	...
	private function _readLabel()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
			$cell->setValueExplicit($value, Generate_PHPExcel_Cell_DataType::TYPE_STRING);
	...
	private function _readBlank()
	{
		...
		$columnString = Generate_PHPExcel_Cell::stringFromColumnIndex($col);
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($isActive) {
			$this->_Generate_PHPExcel->setActiveSheetIndex($this->_Generate_PHPExcel->getIndex($this->_phpSheet));
		}
		...
		if ($this->_phpSheet->getSheetView()->getView() !== Generate_PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
			...
			$view = $isPageBreakPreview? Generate_PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
				Generate_PHPExcel_Worksheet_SheetView::SHEETVIEW_NORMAL;
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(Generate_PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(Generate_PHPExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
			} else {
				// unfrozen panes; split windows; not supported by Generate_PHPExcel core
			}
		}
	}
	...
	private function _includeCellRangeFiltered($cellRangeAddress)
	{
		...
		if ($this->getReadFilter() !== NULL) {
			...
			$rangeBoundaries = Generate_PHPExcel_Cell::getRangeBoundaries($cellRangeAddress);
			$rangeBoundaries[1][0]++;
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			foreach (Generate_PHPExcel_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = Generate_PHPExcel_Cell_DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = Generate_PHPExcel_Cell_DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = Generate_PHPExcel_Cell_DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = Generate_PHPExcel_Cell_DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = Generate_PHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}
		...
		try {
			...
			if ($type == Generate_PHPExcel_Cell_DataValidation::TYPE_LIST) {
		...
		foreach ($cellRangeAddresses as $cellRange) {
			...
			$stRange = Generate_PHPExcel_Cell::extractAllCellReferencesInRange($stRange);
...
//			var_dump($bcBitCount);
			...
			$drawing = new Generate_PHPExcel_Worksheet_Drawing();
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x19:	//	Special attribute
			...
			case 0x41:
				...
				case 0x05:
					...
					break;
				default:
					throw new Generate_PHPExcel_Reader_Exception('Unrecognized space type in tAttrSpace token');
					break;
				}
				...
				break;
			default:
				throw new Generate_PHPExcel_Reader_Exception('Unrecognized attribute flag in tAttr token');
				break;
			}
			break;
		...
		case 0x61:
			...
			default:
				throw new Generate_PHPExcel_Reader_Exception('Unrecognized function in formula');
				break;
			}
		...
		default:
			throw new Generate_PHPExcel_Reader_Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
			break;
		}
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = Generate_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));
	...
	{
		list($baseCol, $baseRow) = Generate_PHPExcel_Cell::coordinateFromString($baseCell);
		$baseCol = Generate_PHPExcel_Cell::columnIndexFromString($baseCol) - 1;
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = Generate_PHPExcel_Cell::stringFromColumnIndex($colIndex);
				$column = '$' . $column;
			} else {
				$colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
				$column = Generate_PHPExcel_Cell::stringFromColumnIndex($baseCol + $colIndex);
			}
	...
	private function _readBIFF5CellRangeAddressFixed($subData)
	{
		...
		// check values
		if ($fr > $lr || $fc > $lc) {
			throw new Generate_PHPExcel_Reader_Exception('Not a cell range address');
		}

		// column index to letter
		$fc = Generate_PHPExcel_Cell::stringFromColumnIndex($fc);
		$lc = Generate_PHPExcel_Cell::stringFromColumnIndex($lc);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// bit: 7-0; mask 0x00FF; column index
		$fc = Generate_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 4));
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = Generate_PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		if (!(0x4000 & self::_GetInt2d($subData, 4))) {
			...
			$fc = Generate_PHPExcel_Cell::stringFromColumnIndex($fcIndex);
			$fc = '$' . $fc;
		} else {
			// column offset
			$fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
			$fc = Generate_PHPExcel_Cell::stringFromColumnIndex($baseCol + $fcIndex);
		}
		...
		$lc = Generate_PHPExcel_Cell::stringFromColumnIndex($baseCol + $lcIndex);

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d($subData, 6))) {
			// absolute column index
			$lc = Generate_PHPExcel_Cell::stringFromColumnIndex($lcIndex);
			$lc = '$' . $lc;
		} else {
			// column offset
			$lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
	 ...
	 * in which case an Generate_PHPExcel_Reader_Exception is thrown
	 *
	 * @param int $index
	 * @return string|false
	 * @throws Generate_PHPExcel_Reader_Exception
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
						throw new Generate_PHPExcel_Reader_Exception('Deleted sheet reference');
					}
				...
				default:
					...
					throw new Generate_PHPExcel_Reader_Exception('Excel5 reader only supports internal sheets in fomulas');
					break;
			}
		}
		return false;
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return Generate_PHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return Generate_PHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return Generate_PHPExcel_Style_Border::BORDER_NONE;
			case 0x01: return Generate_PHPExcel_Style_Border::BORDER_THIN;
			case 0x02: return Generate_PHPExcel_Style_Border::BORDER_MEDIUM;
			case 0x03: return Generate_PHPExcel_Style_Border::BORDER_DASHED;
			case 0x04: return Generate_PHPExcel_Style_Border::BORDER_DOTTED;
			case 0x05: return Generate_PHPExcel_Style_Border::BORDER_THICK;
			case 0x06: return Generate_PHPExcel_Style_Border::BORDER_DOUBLE;
			case 0x07: return Generate_PHPExcel_Style_Border::BORDER_HAIR;
			case 0x08: return Generate_PHPExcel_Style_Border::BORDER_MEDIUMDASHED;
			case 0x09: return Generate_PHPExcel_Style_Border::BORDER_DASHDOT;
			case 0x0A: return Generate_PHPExcel_Style_Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return Generate_PHPExcel_Style_Border::BORDER_DASHDOTDOT;
			case 0x0C: return Generate_PHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return Generate_PHPExcel_Style_Border::BORDER_SLANTDASHDOT;
			default:   return Generate_PHPExcel_Style_Border::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return Generate_PHPExcel_Style_Fill::FILL_NONE;
			case 0x01: return Generate_PHPExcel_Style_Fill::FILL_SOLID;
			case 0x02: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKUP;
			case 0x09: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_GRAY125;
			case 0x12: return Generate_PHPExcel_Style_Fill::FILL_PATTERN_GRAY0625;
			default:   return Generate_PHPExcel_Style_Fill::FILL_NONE;
		}
	}
	...
	private function _parseRichText($is = '') {
		$value = new Generate_PHPExcel_RichText();
