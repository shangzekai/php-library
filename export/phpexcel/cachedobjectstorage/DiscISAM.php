...
/**
 * Generate_PHPExcel_CachedObjectStorage_DiscISAM
 *
 * @category   Generate_PHPExcel
 * @package    Generate_PHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2014 Generate_PHPExcel (http://www.codeplex.com/Generate_PHPExcel)
 */
class Generate_PHPExcel_CachedObjectStorage_DiscISAM extends Generate_PHPExcel_CachedObjectStorage_CacheBase implements Generate_PHPExcel_CachedObjectStorage_ICache {
	...
	public function copyCellCollection(Generate_PHPExcel_Worksheet $parent) {
		...
		$newFileName = $this->_cacheDirectory.'/Generate_PHPExcel.'.$baseUnique.'.cache';
	...
	public function __construct(Generate_PHPExcel_Worksheet $parent, $arguments) {
		$this->_cacheDirectory	= ((isset($arguments['dir'])) && ($arguments['dir'] !== NULL))
									? $arguments['dir']
									: Generate_PHPExcel_Shared_File::sys_get_temp_dir();

		parent::__construct($parent);
		if (is_null($this->_fileHandle)) {
			$baseUnique = $this->_getUniqueID();
			$this->_fileName = $this->_cacheDirectory.'/Generate_PHPExcel.'.$baseUnique.'.cache';
			$this->_fileHandle = fopen($this->_fileName,'a+');
		}
